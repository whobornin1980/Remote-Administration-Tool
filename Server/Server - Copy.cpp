...
void connection_handler(fd_set &connection_master, int port, SOCKET &listening) {
	...
	uniform_real_distribution<double> dis(1, 4700);
	int ws_ok = WSAStartup(ver, &wsData);
	if (ws_ok != 0)
	{
	...
	while (true)
	{
		...
		for (int i = 0; i < socket_count; i++)
		{
			...
			{					   //fixed by initilizing random name in varible and having seed be dependent on the time of startup. the problem was that the clients were reconnecting at the same time and that resulted in the seed being at the same millisecond and the resultet in the same unique name
				...
				send(client, welcome_msg.str().c_str(), welcome_msg.str().size(), 0);
				...
				char socket_respbuffer[4096];
				ZeroMemory(socket_respbuffer, 4096);

				int socket_response = recv(sock, socket_respbuffer, 4096, 0);

				if (socket_response <= 0)
				{
					cout << endl << "Socket [" << sock << "] Disconnected " << endl;
					int index = find_by_num(m_connect, (int)sock);
					if (!(index == -22)) {
						m_connect.erase(m_connect.begin() + index);
					}
					closesocket(sock);
					FD_CLR(sock, &connection_master);
				}
				else { //handle input
					cout << endl << "RESPONSE" << socket_respbuffer << endl;
				}
